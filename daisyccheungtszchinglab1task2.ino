...
void setup() {
  ...
  Serial.println("Vout (mV), I (uA)"); // display duty cycle,Vout for TASK 1 or I,Vout for Task 2 3 4
  for (int onPeriod = 0; onPeriod < 256; onPeriod++) 
    ...
    c1 = float(Vout_value*3.3)/4095  ;
    c2 = float(c_value*3.3)/4095;//calculate the voltage at the node for current calcultion  
    x = c1 * 1000.0; // calculate duty cycle for TASK 1 or the voltage at Vout in mV for Task 2 3 4
    y =  (c1 - c2) / R; // calculate the voltage at Vout in V for TASK 1 or current in uA for Task 2 3 4
    Serial.print(x,1); 
